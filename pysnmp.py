...
def monta_snmp(oid):
	...
	version = chr(2) + chr(1) + chr(0)
	
	##Snmp Message
	msg_type = chr(0x30)
	snmp = msg_type + chr(3 + 2 + len_community + len(spdu)) + version + comm + spdu
	return snmp

def send_socket_msg(message, host='127.0.0.1'):
	
	s = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
	s.settimeout(10)

	s.sendto(message.encode(), (host, 161))
